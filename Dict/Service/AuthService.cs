...
namespace Dict.Service
{
    public class AuthService : IAuthService
    {
        ...
        private string? ValidatePassword(string password)
        {
            ...
            // --- Bạn có thể tùy chỉnh các quy tắc bên dưới ---

            // Quy tắc 1: Độ dài tối thiểu
            if (password.Length < 8)
            {
                return "Password must be at least 8 characters long.";
            }

            // Quy tắc 2: Phải có ít nhất 1 chữ hoa
            if (!password.Any(char.IsUpper))
            {
                return "Password must contain at least one uppercase letter.";
            }

            // Quy tắc 3: Phải có ít nhất 1 chữ thường
            if (!password.Any(char.IsLower))
            {
                return "Password must contain at least one lowercase letter.";
            }

            // Quy tắc 4: Phải có ít nhất 1 số
            if (!password.Any(char.IsDigit))
            {
                return "Password must contain at least one number.";
            }

            // Quy tắc 5: Phải có ít nhất 1 ký tự đặc biệt
            // (Bạn có thể thêm/bớt các ký tự trong chuỗi này)
            ...
            // Nếu vượt qua tất cả, trả về null (không có lỗi)
            return null;
        }
