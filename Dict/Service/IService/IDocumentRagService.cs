...
namespace Dict.Service.IService
{
    public interface IDocumentRagService
    {
        ...
        IAsyncEnumerable<RagStreamEvent> AskWorkspaceStreamAsync(int workspaceId, int userId, string question, int topK = 5, List<DocumentRagTurnDto>? history = null, string? sessionId = null, string mode = "high", bool skipClarify = false);
        IAsyncEnumerable<RagStreamEvent> AskProjectStreamAsync(int projectId, int userId, string question, int topK = 5, List<DocumentRagTurnDto>? history = null, string? sessionId = null, string mode = "high", bool skipClarify = false);
    }
}
