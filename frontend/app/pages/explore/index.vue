<template>
  <div id="app" class="bg-gray-900 min-h-screen relative">

    <div v-if="showLoginNotice" class="fixed top-5 right-5 bg-yellow-500 text-gray-900 py-2 px-4 rounded-lg shadow-lg z-[100] animate-fade-in-out">
        Vui lòng đăng nhập để thực hiện!
    </div>
     <div v-if="showSaveSuccessGlobal" class="fixed bottom-5 right-5 bg-green-600 text-white py-2 px-4 rounded-lg shadow-lg animate-bounce z-[100]">
      Đã lưu vào Sổ tay!
    </div>
...
type View = 'home' | 'list' | 'review' | 'edit-deck' | 'create-deck' | 'my-decks-list' | 'explore-list';
...
async function fetchDeckDetail(id: number): Promise<DeckDetailDto | null> {
    ...
    try {
       const response = await fetch(`${BASE_URL}/api/decks/${id}`, {
            cache: 'no-store',
            headers: isAuthenticated.value ? { 'Authorization': `Bearer ${jwt.value}` } : {}
        });
...
function startReview(mode: LearningMode) { if(selectedSet.value) { learningMode.value = mode; currentView.value = 'review';} }
function goHome() { currentView.value = 'home'; selectedSet.value = null; }
...
function goToEditDeck() { currentView.value = 'edit-deck'; }
function goToCreateDeck() { currentView.value = 'create-deck'; }
function goToMyDecksList() { currentView.value = 'my-decks-list'; }
function goToExploreList() { currentView.value = 'explore-list'; }
...
async function displaySaveSuccessAndRefreshMyDecks() {
    ...
    saveSuccessTimeout = setTimeout(() => { showSaveSuccessGlobal.value = false; saveSuccessTimeout = null; }, 2500);
    await refreshMyDecksInBackground();
}
...
@keyframes bounceSimple { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-10px); } }
.animate-bounce { animation: bounceSimple 1s ease-in-out; }
</style>