<template>
  <div
    class="min-h-screen flex flex-col items-center justify-center p-4 bg-gray-100 text-gray-900 dark:bg-gray-900 dark:text-white transition-colors"
  >
    <div class="w-full max-w-2xl">
      <div class="flex justify-between items-center mb-4">
        ...
        <div class="flex items-center gap-2">
          <span class="text-sm text-gray-600 dark:text-gray-400">Lọc:</span>
          <button
            @click="setFilter('all')"
            :class="['filter-btn', { 'filter-active': filter === 'all' }]"
          >
            Tất cả
          </button>
          <button
            @click="setFilter('not-remembered')"
            :class="[
              'filter-btn',
              { 'filter-active': filter === 'not-remembered' },
            ]"
          >
            Chưa nhớ
          </button>
          <button
            @click="setFilter('remembered')"
            :class="[
              'filter-btn',
              { 'filter-active': filter === 'remembered' },
            ]"
          >
            Đã nhớ
          </button>
      ...
      <div
        v-if="currentCard"
        class="bg-white dark:bg-gray-800 rounded-lg p-6 relative shadow-lg"
      >
        ...
        <div
          class="absolute bottom-4 left-6 text-sm text-gray-400 dark:text-gray-500"
        >
      ...
      <div
        v-else
        class="text-center text-gray-400 dark:text-gray-500 p-10 bg-gray-100 dark:bg-gray-800 rounded-lg"
      >
      ...
      <div class="flex justify-center gap-4 mt-6">
        <button
          @click="markAs('not-remembered')"
          class="control-btn bg-red-600 hover:bg-red-700"
        >
          Chưa nhớ
        </button>
        <button
          @click="markAs('remembered')"
