<template>
  <client-only>
    <VueForceGraph2D
      ...
      :linkDirectionalArrowColor="() => colorMode.value === 'dark' ? '#ffffff' : '#000000'"
      :linkDirectionalArrowRelPos="linkArrowRelPos"
      :linkDirectionalParticles="3"
      :linkDirectionalParticleSpeed="0.004"
      :linkDirectionalParticleWidth="() => props.showParticles ? 2 : 0"
      :linkDirectionalParticleColor="() => colorMode.value === 'dark' ? '#ffffff' : '#000000'"
...
import type { NodeObject, LinkObject, GraphData, BothGraphData } from '~/types/graph'
...
const fgRef = ref<{
  zoomToFit?: (ms?: number, paddingPx?: number, filter?: (n: NodeObject) => boolean) => void
} | null>(null)
...
const jinmeiyoList = kanjilist.filter((el) => el.g === 2).map((el) => String(el.k))
...
watch(
  () => [data.value.nodes.length, props.kanjiInfo?.id, props.triggerFocus, props.bounds?.width],
  () => {
    if (props.kanjiInfo?.id && data.value.nodes.length > 0) {
      setTimeout(() => fgRef.value?.zoomToFit?.(1000, props.bounds.width * 0.1), 100)
    }
  }
)
...
// node label
const nodeLabelFn = (n: NodeObject) => `${(n.data as any)?.jishoData?.kunyomi ?? ''}<br/>${(n.data as any)?.jishoData?.meaning ?? ''}`
...
function paintNode(node: NodeObject, ctx: CanvasRenderingContext2D) {
  ...
  let color = '#fff'
  if (props.kanjiInfo?.id === node.id) color = '#2B99CF'
  else if (joyoList.includes(String(node.id))) color = '#80c2e2'
  else if (jinmeiyoList.includes(String(node.id))) color = '#d5ebf5'
  if (node.id === hoverNode.value?.id) color = '#2B99CF'
...
function nodePointerAreaPaint(node: NodeObject, color: string, ctx: CanvasRenderingContext2D) {
  const label = String(node.id)
  const fontSize = 6
  ctx.font = `${fontSize}px Sans-Serif`
  const textWidth = ctx.measureText(label).width
...
function linkCanvasObject(link: LinkObject, ctx: CanvasRenderingContext2D) {
  ...
  if (typeof s === 'object' && typeof t === 'object' && s.x != null && s.y != null && t.x != null && t.y != null) {
    const sx = s.x
    const sy = s.y
    const tx = t.x
    const ty = t.y
    const midX = (sx + tx) / 2
    const midY = (sy + ty) / 2
    const linkText = sameOn(String((s as NodeObject).id), String((t as NodeObject).id))

    ctx.beginPath()
    ctx.moveTo(sx, sy)
    ctx.lineTo(tx, ty)
    ctx.lineWidth = 0.25
    ctx.strokeStyle = colorMode.value === 'dark' ? '#ffffff' : '#000000'
    ctx.stroke()
...
function linkArrowRelPos({ source, target }: { source: any; target: any }) {
  if (typeof source === 'object' && typeof target === 'object' && source.x != null && target.x != null && source.y != null && target.y != null) {
...
const linkColor = () => {
  if (typeof document === 'undefined') return colorMode.value === 'dark' ? '#ffffff' : '#000000'
  return getComputedStyle(document.body).getPropertyValue('--color-foreground') || (colorMode.value === 'dark' ? '#ffffff' : '#000000')
}
</script>
