  ...
  >
    <div class="max-w-7xl mx-auto">
      <div class="mb-6 flex items-center justify-between">
        ...
        <h1 class="text-2xl font-bold text-primary-600 dark:text-sky-400">
          Sổ tay của tôi
        </h1>
        <div></div>
      </div>
      ...
      >
        <div class="relative w-full sm:max-w-xs">
          <input
            ...
            class="w-full bg-white text-gray-900 border border-gray-300 rounded-lg pl-10 pr-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:focus:ring-sky-500"
          />
          <svg
        ...
        <button
          ...
          class="bg-primary-600 hover:bg-primary-700 dark:bg-sky-600 dark:hover:bg-sky-700 text-white font-bold py-2 px-5 rounded-lg flex items-center w-full sm:w-auto shrink-0"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="h-5 w-5 mr-2"
      ...
      <div
        v-if="filteredDecks.length === 0 && !searchTerm"
      ...
      <div
        ...
        class="text-center text-gray-600 dark:text-gray-500 py-20"
      >
        Không tìm thấy sổ tay nào khớp với "{{ searchTerm }}".
      </div>
      ...
      >
        ...
        <span v-for="page in pageNumbers" :key="page">
          <button
            ...
            :class="[
              'pagination-btn',
              { 'bg-sky-600 text-white': currentPage === page },
            ]"
          >
            {{ page }}
          </button>
...
<style scoped>
/* THAY ĐỔI:
  - Tái cấu trúc lại style cho pagination
*/
...
/* --- ACTIVE STATE (Cho cả 2 chế độ) --- */
/* (Class 'bg-sky-600' được áp dụng từ template) */

/* Active - Light */
.pagination-btn.bg-sky-600 {
...
/* Active - Dark */
.dark .pagination-btn.bg-sky-600 {
  background-color: #0284c7; /* sky-600 (original) */
  color: white;
}
</style>