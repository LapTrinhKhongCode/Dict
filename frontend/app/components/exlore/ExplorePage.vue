  ...
  <div class="min-h-screen text-white p-4 sm:p-8">
    <div class="max-w-7xl mx-auto">
      <div class="mb-6 flex items-center justify-between">
         <button @click="emit('go-back')" class="flex items-center text-sm text-emerald-400 hover:text-emerald-300 transition-colors">
          &larr; Quay lại trang chủ
        </button>
         <h1 class="text-2xl font-bold text-emerald-400">Khám phá bộ thẻ</h1>
        <div></div>
      </div>
      ...
      <div class="mb-8 max-w-lg mx-auto">
         <div class="relative">
             <input
                ...
                class="w-full bg-gray-800 border border-gray-600 rounded-full pl-12 pr-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-emerald-500 text-lg"
              />
              <svg class="absolute left-4 top-1/2 transform -translate-y-1/2 h-6 w-6 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                 <path fill-rule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z" clip-rule="evenodd" />
              </svg>
          </div>
      </div>

      <div v-if="isSearching" class="text-center text-gray-400 py-20">Đang tìm kiếm...</div>
      <div v-else-if="error" class="text-center text-red-400 p-4 bg-red-900/50 rounded-lg">{{ error }}</div>
      <div v-else-if="filteredDecks.length === 0 && !searchTerm" class="text-center text-gray-500 py-20">Không có bộ thẻ công khai nào.</div>
      <div v-else-if="filteredDecks.length === 0 && searchTerm" class="text-center text-gray-500 py-20">Không tìm thấy bộ thẻ nào khớp với "{{ searchTerm }}".</div>
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
       ...
       <div v-if="totalPages > 1 && !isSearching" class="mt-8 flex justify-center items-center space-x-2 text-gray-400">
          ...
          <span v-for="page in pageNumbers" :key="page">
              <button
                  ...
                  @click="goToPage(page)"
                  :class="['pagination-btn', { 'bg-emerald-600 text-white': currentPage === page }]"
              >
                  {{ page }}
              </button>
...
const pageNumbers = computed(() => {
    ...
    for (const i of uniqueRange) {
        if (l !== undefined) { if (i - l === 2) { rangeWithDots.push(l + 1); } else if (i - l !== 1) { rangeWithDots.push('...'); } }
...
.pagination-btn.bg-emerald-600 { /* Active page style for Explore */
