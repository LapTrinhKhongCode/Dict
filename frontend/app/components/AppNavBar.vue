...
const currentNotification = computed(
  () => notifications[currentNotificationIndex.value],
);
...
const targetDateString = computed(() =>
  nextJlptDate.value.toLocaleDateString("vi-VN"),
);
...
async function handleRespond(invitationId: number, accept: boolean) {
  try {
    ...
    showToast(
      accept ? "Bạn đã tham gia Workspace!" : "Đã từ chối lời mời.",
      "success",
    );
...
<template>
  <header
    class="bg-white dark:bg-neutral-900 px-6 py-3 flex items-center justify-between border-b border-gray-200 dark:border-neutral-800 transition-colors sticky top-0 z-[100]"
  >
    <div class="flex items-center space-x-10">
      ...
      <div class="relative flex items-center cursor-pointer group">
        ...
        >
        <div
          class="absolute top-10 left-20 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition duration-300 bg-primary-600 text-white text-sm px-3 py-1 rounded-lg whitespace-nowrap shadow-lg z-50"
        >
    ...
    <div class="flex-1 flex justify-center items-center px-4">
      ...
      >
        <div
          class="relative h-6 overflow-hidden text-gray-500 dark:text-gray-400 text-sm group-hover:opacity-0 transition-opacity duration-300 text-center"
        >
          <Transition name="slide-up">
            <span
              :key="currentNotification"
              class="notification-text glow flex items-center justify-center w-full absolute inset-0"
            >
              <UIcon
        ...
        >
          ...
          >
            ...
            >
              <UIcon
                name="i-lucide-sparkles"
                class="mr-3 text-primary-500 flex-shrink-0"
              />
    ...
    <div class="flex items-center space-x-4 relative">
      <span
        ...
        class="text-gray-700 dark:text-gray-300 whitespace-nowrap hidden sm:block"
      >
      ...
      <div v-if="isAuthenticated" class="relative">
        ...
        <div
          v-if="showNotifications"
          class="absolute right-0 top-full mt-2 w-80 bg-white dark:bg-neutral-800 border border-gray-200 dark:border-neutral-700 rounded-xl shadow-2xl z-50 overflow-hidden"
        >
          ...
          >
            <li
              ...
              :key="inv.id"
              class="p-4 border-b hover:bg-gray-50 dark:hover:bg-neutral-700/50 transition-colors"
            >
              <div class="flex gap-3">
                <div
                  class="w-10 h-10 rounded-lg bg-blue-100 dark:bg-blue-900/30 text-blue-600 flex items-center justify-center font-bold text-lg flex-shrink-0"
                >
                  {{ inv.workspaceName?.[0]?.toUpperCase() || "W" }}
                </div>
                <div class="flex-1">
                  <p class="text-sm">
                    <strong>{{ inv.inviterName }}</strong> mời tham gia
                    <strong>{{ inv.workspaceName }}</strong>
                  </p>
                  <div class="flex gap-2 mt-3">
                    <button
                      @click="handleRespond(inv.id, true)"
                      class="flex-1 bg-yellow-400 text-black text-xs font-bold py-1.5 rounded-lg"
                    >
                      Đồng ý
                    </button>
                    <button
                      @click="handleRespond(inv.id, false)"
                      class="flex-1 bg-gray-200 dark:bg-neutral-700 text-xs font-bold py-1.5 rounded-lg"
                    >
                      Từ chối
                    </button>
                  </div>
